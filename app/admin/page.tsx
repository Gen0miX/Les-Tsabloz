...
import {
  BookingCard,
  BookingCardSkeleton,
} from "@/components/admin/booking-card";
...
const SECTIONS: {
...
}[] = [
  {
    ...
    status: "pending",
    numeral: "§ 01",
    heading: "Demandes en attente",
  },
  {
    label: "Confirmées",
    status: "approved",
    numeral: "§ 02",
    heading: "Séjours confirmés",
  },
  {
    label: "Refusées",
    status: "rejected",
    numeral: "§ 03",
    heading: "Demandes refusées",
  },
  {
    label: "Terminées",
    status: "completed",
    numeral: "§ 04",
    heading: "Séjours terminés",
  },
];
...
export default function AdminPage() {
  ...
  const counts = useMemo(
    () => ({
      ...
      approved: bookings.filter(
        (b) => b.status === "approved" && !isCompleted(b),
      ).length,
  ...
  return (
    <div className="lt-root min-h-screen bg-(--lt-bg) grid md:grid-cols-[240px_1fr]">
      {/* Sidebar */}
      <aside className="border-r border-(--lt-line) bg-(--lt-surface) p-5 flex flex-col gap-6">
        <LTWordmark subtle />
        ...
        <nav className="flex flex-col gap-0.5">
          ...
          {SECTIONS.map(({ label, status }) => (
            <button
              ...
              style={{
                background:
                  filter === status ? "var(--lt-surface-2)" : "transparent",
                color:
                  filter === status ? "var(--lt-ink)" : "var(--lt-ink-soft)",
                fontWeight: filter === status ? 500 : 400,
              }}
            >
              <span className="flex items-center gap-2">
                {filter === status && (
                  <span className="w-[3px] h-3.5 bg-(--lt-moss) rounded-sm" />
                )}
                {label}
              </span>
        ...
        <div className="mt-auto flex flex-col gap-1.5">
          ...
          <div className="rounded-(--lt-radius-lg) border border-(--lt-line) bg-(--lt-surface) p-3 text-xs text-(--lt-ink-soft) leading-relaxed">
      ...
      <div className="flex flex-col">
        ...
        <div className="px-9 py-7 border-b border-(--lt-line-soft) grid grid-cols-2 md:grid-cols-4 gap-3.5">
          {[
            ...
            <div
              key={i}
              className="rounded-(--lt-radius-lg) border border-(--lt-line) bg-(--lt-surface) p-4"
            >
        ...
        <main className="px-9 py-7 flex-1">
          <div className="flex justify-between items-center mb-4 gap-4">
            <div>
              ...
              <h2 className="lt-display text-[22px] mt-1">
                {section.heading}
                <span className="text-(--lt-ink-mute) font-light ml-2.5">
                  {visible.length}
                </span>
              </h2>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <select
                ...
                className="h-8 px-2 text-sm rounded-md border border-(--lt-line) bg-(--lt-surface) text-(--lt-ink) focus:outline-none focus:border-(--lt-moss) cursor-pointer"
              >
          ...
          {loadingBookings ? (
            ...
            <div className="rounded-(--lt-radius-lg) border border-(--lt-line) bg-(--lt-surface) p-10 text-center">
