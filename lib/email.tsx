  ...
  return (
    ...
    >
      {pairs.map((pair, i) => (
        <Row
          key={i}
          style={{ marginBottom: i < pairs.length - 1 ? "12px" : "0" }}
        >
          {pair.map((item) => (
...
export function ConfirmationEmail({ booking }: { booking: Booking }) {
  ...
  return (
    <Html lang="fr">
      <Head />
      <Preview>Ta réservation aux Tsabloz est confirmée !</Preview>
      <Body
        style={{
      ...
      >
        <Container style={{ maxWidth: "520px", margin: "0 auto" }}>
          ...
          >
            ...
            >
              Nous avons le plaisir de confirmer ta réservation aux Tsabloz.
      ...
      >
        <Container style={{ maxWidth: "520px", margin: "0 auto" }}>
          ...
          >
            ...
            >
              Une nouvelle demande de réservation a été soumise et attend ta
              validation.
            </Text>
            <InfoGrid
              items={[
...
export async function sendBookingConfirmation(booking: Booking): Promise<void> {
  const html = await render(
    React.createElement(ConfirmationEmail, { booking }),
  );
...
export async function sendBookingNotification(booking: Booking): Promise<void> {
  ...
  const html = await render(
    React.createElement(AdminNotificationEmail, { booking, adminUrl }),
  );
  await getResend().emails.send({
    from: process.env.RESEND_FROM_EMAIL!,
